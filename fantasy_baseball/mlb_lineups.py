...
def pregame_lineups(stats_years):
    """
    Generates a list of game features to be used throughout the simulator. There are 4 pieces: a list of game matchups, the team's batting lineup, the pitching matchup, and that gambling lines

    Parameters
    ----------
    stats_years: list
        List of years you would like player statistics for

    Returns
    -------
    list[list, list, list, list]
        matchup list, team daily lineups w/ batting statistics, team pitching matchups w/ pitching statistics, game gamling lines (moneyline, spread, over/under)

    """

    today = datetime.now().strftime("%Y-%m-%d")

    stats_list = stats(stats_years)

    player_bat_df = stats_list[0]
    player_pitch_df = stats_list[1]
    ...
    return [matchup_list, lineup_stats, pitching_matchup_stats, game_gambling]
